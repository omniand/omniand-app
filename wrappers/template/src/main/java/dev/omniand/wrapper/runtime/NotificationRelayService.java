...
public final class NotificationRelayService extends Service {
    ...
    private final Binder binder = new Binder() {
        @Override protected boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            ...
            if (!trustedCaller()) { reply.writeException(new SecurityException("Untrusted caller")); return true; }
            try {
                String requestedApp = data.readString();
                if (!appId.equals(requestedApp)) throw new SecurityException("Wrong app id");
                boolean ok;
                if (code == 1) {
                    String thread = data.readString(); int id = data.readInt(); String title = data.readString();
                    String preview = data.readString(); long timestamp = data.readLong();
                    ok = publish(thread, id, title, preview, timestamp);
                } else if (code == 2) {
                    String thread = data.readString(); cancel(thread.hashCode() & 0x7fffffff); ok = true;
                } else if (code == 3) { manager().cancelAll(); ok = true; }
                else return super.onTransact(code, data, reply, flags);
                reply.writeNoException(); reply.writeInt(ok ? 1 : 0); return true;
    ...
    private boolean trustedCaller() {
        ...
        for (String name : packages) if ("dev.omniand.launcher".equals(name) && platformCertificate.equals(fingerprint(name))) return true;
        return false;
    }

    @SuppressWarnings("deprecation") private String fingerprint(String packageName) {
        try {
            int flags = Build.VERSION.SDK_INT >= 28 ? PackageManager.GET_SIGNING_CERTIFICATES : PackageManager.GET_SIGNATURES;
            PackageInfo info = getPackageManager().getPackageInfo(packageName, flags);
            byte[] certificate = Build.VERSION.SDK_INT >= 28 ? info.signingInfo.getApkContentsSigners()[0].toByteArray() : info.signatures[0].toByteArray();
            byte[] digest = MessageDigest.getInstance("SHA-256").digest(certificate);
            StringBuilder value = new StringBuilder(); for (byte item : digest) value.append(String.format("%02x", item));
    ...
    private boolean publish(String thread, int id, String title, String preview, long timestamp) {
        ...
        android.app.Notification notification = new NotificationCompat.Builder(this, CHANNEL)
            .setSmallIcon(android.R.drawable.sym_action_chat).setContentTitle(title).setContentText(preview).setWhen(timestamp)
            .setAutoCancel(true).setVisibility(NotificationCompat.VISIBILITY_PRIVATE).setPublicVersion(publicBuilder.build())
            .setContentIntent(pending).build();
        manager().notify(id, notification); return true;
    }

    private NotificationManager manager() { return getSystemService(NotificationManager.class); }
    private void cancel(int id) { manager().cancel(id); }
}
