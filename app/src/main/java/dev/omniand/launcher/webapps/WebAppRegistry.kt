...
object WebAppRegistry {
    ...
    private fun fromDirectory(directory: File): WebApp? =
        runCatching {
                ...
                val iconPath =
                    manifest.optString("icon").takeIf {
                ...
                WebApp(
                    ...
                    buildSet {
                        if (permissions != null)
                            for (index in 0 until permissions.length()) add(
                                permissions.getString(index)
                            )
                    },
                    fileRoot = directory,
                    iconPath = iconPath?.takeIf { File(directory, it).isFile },
                )
            }
            .getOrNull()
