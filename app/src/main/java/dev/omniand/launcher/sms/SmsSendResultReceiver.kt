...
data class SmsSendProgress(val partCount: Int, val completedParts: Set<Int> = emptySet(), val failed: Boolean = false)

enum class SmsSendOutcome { PENDING, SENT, FAILED, DUPLICATE }

object SmsSendReducer {
    fun record(progress: SmsSendProgress, part: Int, succeeded: Boolean): Pair<SmsSendProgress, SmsSendOutcome> {
        ...
        val updated = progress.copy(completedParts = progress.completedParts + part, failed = progress.failed || !succeeded)
        val outcome = if (updated.completedParts.size < updated.partCount) SmsSendOutcome.PENDING
...
object SmsSendTracker {
    ...
    @Synchronized fun start(context: Context, messageId: String, partCount: Int) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
            .putString(messageId, encode(SmsSendProgress(partCount))).commit()
    }

    @Synchronized fun record(context: Context, messageId: String, part: Int, partCount: Int, succeeded: Boolean): SmsSendOutcome {
        val preferences = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val current = preferences.getString(messageId, null)?.let(::decode) ?: SmsSendProgress(partCount)
        ...
        if (outcome == SmsSendOutcome.SENT || outcome == SmsSendOutcome.FAILED) preferences.edit().remove(messageId).commit()
        else if (outcome != SmsSendOutcome.DUPLICATE) preferences.edit().putString(messageId, encode(updated)).commit()
        return outcome
    }
    ...
    fun updateProvider(context: Context, messageId: String, outcome: SmsSendOutcome, errorCode: Int) {
        ...
        context.contentResolver.update(
            ...
            ContentValues().apply {
                put(Telephony.Sms.TYPE, if (succeeded) Telephony.Sms.MESSAGE_TYPE_SENT else Telephony.Sms.MESSAGE_TYPE_FAILED)
                put(Telephony.Sms.STATUS, if (succeeded) Telephony.Sms.STATUS_COMPLETE else Telephony.Sms.STATUS_FAILED)
    ...
    private fun encode(progress: SmsSendProgress) = "${progress.partCount}|${if (progress.failed) 1 else 0}|${progress.completedParts.sorted().joinToString(",")}" 
    private fun decode(value: String): SmsSendProgress {
        ...
        val completed = pieces.getOrNull(2).orEmpty().split(',').mapNotNull(String::toIntOrNull).toSet()
...
class SmsSendResultReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        val messageId = intent.getStringExtra(EXTRA_MESSAGE_ID)?.takeIf { it.toLongOrNull() != null } ?: return
