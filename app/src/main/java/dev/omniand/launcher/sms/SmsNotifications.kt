...
object SmsNotifications {
    ...
    fun publish(context: Context, threadId: String, title: String, preview: String, timestamp: Long) {
        if (WrapperNotificationRelay.publish(context, threadId, notificationId(threadId), title, preview, timestamp)) return
        ensureChannel(context)
        if (Build.VERSION.SDK_INT >= 33 && context.checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) return
        val messagesInstalled = runCatching { dev.omniand.launcher.webapps.WebAppRegistry.apps(context).any { it.id == "messages" } }.getOrDefault(false)
        val click = if (messagesInstalled) Intent(context, WebAppActivity::class.java)
        ...
        val pending = PendingIntent.getActivity(context, notificationId(threadId), click,
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL)
            ...
            .setPublicVersion(NotificationCompat.Builder(context, CHANNEL)
                .setSmallIcon(android.R.drawable.sym_action_chat).setContentTitle("Messages")
                .setContentText("New message").build())
    ...
    fun publishMmsUnsupported(context: Context, timestamp: Long) =
        publish(context, "mms-$timestamp", "Messages", "MMS received — display not supported", timestamp)
    ...
    private fun ensureChannel(context: Context) {
        if (Build.VERSION.SDK_INT >= 26) context.getSystemService(NotificationManager::class.java)
            .createNotificationChannel(NotificationChannel(CHANNEL, "Messages", NotificationManager.IMPORTANCE_HIGH))
    }
