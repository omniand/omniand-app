...
object IncomingSmsAssembler {
    fun assemble(parts: List<IncomingPart>): IncomingPart? {
        ...
        return first.copy(
            ...
            timestamp = parts.minOf { it.timestamp },
        )
    }
}
...
class IncomingSmsReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        EXECUTOR.execute {
            try {
                persist(context.applicationContext, intent)
            } finally {
                pending.finish()
            }
        }
    ...
    private fun persist(context: Context, intent: Intent) {
        ...
        val message =
            IncomingSmsAssembler.assemble(
                messages.map {
                    IncomingPart(
                        it.displayOriginatingAddress.orEmpty(),
        ...
        val fingerprint =
            sha256(
                "${message.address}\u0000${message.body}\u0000${message.timestamp}\u0000${message.subscriptionId}"
            )
        ...
        try {
            val uri =
                context.contentResolver.insert(Telephony.Sms.Inbox.CONTENT_URI, values)
                    ?: error("SMS provider rejected insert")
            val id = uri.lastPathSegment.orEmpty()
            val threadId =
                context.contentResolver
                    .query(uri, arrayOf(Telephony.Sms.THREAD_ID), null, null, null)
                    ?.use {
            ...
            SmsNotifications.publish(
                context,
                threadId,
                message.address.ifBlank { "New message" },
                message.body,
                receivedAt,
            )
...
object DedupLedger {
    ...
    @Synchronized
    fun claim(context: Context, fingerprint: String): Boolean {
    ...
    @Synchronized
    fun release(context: Context, fingerprint: String) {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        prefs
            .edit()
            .putString(
                ...
                prefs
                    ...
                    .orEmpty()
                    .lineSequence()
                    .filter { it.isNotBlank() && it != fingerprint }
                    .joinToString("\n"),
            )
            .commit()
    }
